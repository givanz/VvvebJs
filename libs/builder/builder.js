...
*/
    ...
    render: function (type) {
        ...
        jQuery(this.componentPropertiesElement + " .tab-pane").each(
            function () {
        ...
        var fn = function (component, property) {
            return property.input.on("propertyChange", function (
                event,
                value,
                input
            ) {
        ...
        for (var i in component.properties) {
            ...
            if (property.init) {
                ...
                if (property.htmlAttr == "style") {
                    ...
                    var value = Vvveb.StyleManager.getStyle(
                        element,
                        property.key
...
Vvveb.Builder = {
    ...
    loadControlGroups: function () {
        ...
        componentsList.each(function () {
            ...
            for (group in Vvveb.ComponentsGroup) {
                list.append(
                    '<li class="header clearfix" data-section="' +
                        group +
                        '"  data-search=""><label class="header" for="' +
                        type +
                        "_comphead_" +
                        group +
                        count +
                        ...
                        '  <div class="header-arrow"></div>\
									   </label><input class="header_check" type="checkbox" checked="true" id="' +
                        type +
                        "_comphead_" +
                        group +
                        count +
                ...
                for (i in components) {
                    ...
                    if (component) {
                        item = $(
                            '<li data-section="' +
                                group +
                                '" data-drag-type=component data-type="' +
                                componentType +
                                '" data-search="' +
                ...
                for (i in blocks) {
                    ...
                    if (block) {
                        item = $(
                            '<li data-section="' +
                                group +
                                '" data-drag-type=block data-type="' +
                                blockType +
                                '" data-search="' +
        ...
        if (el.attributes)
            for (var j = 0; j < el.attributes.length; j++) {
                if (el.attributes[j].nodeName.indexOf("data-component") > -1) {
                    componentName = el.attributes[j].nodeName.replace(
                        "data-component-",
                        ""
                    );
                }
    ...
    selectNode: function (node) {
        ...
        Vvveb.FileManager.highlightComponent(target);
    },
                    ...
                    try {
                        if (event.originalEvent) {
                            if (
                                ...
                                else self.dragElement.appendTo(parent);
                            } else {
                                if (isIE11)
                                    self.highlightEl.prepend(self.dragElement);
                                else self.dragElement.prependTo(parent);
                            }
    ...
    _initBox: function () {
        ...
        $(".components-list li ol li", addSectionBox).on("click", function (
            ...
            addSectionComponent(
                html,
                jQuery("[name='add-section-insert-mode']:checked").val() ==
                    "after"
            );
    ...
    _initDragdrop: function () {
        ...
        $(".drag-elements-sidepane ul > li > ol > li").on(
            ...
            function (event) {
                ...
                else self.component = Vvveb.Blocks.get($this.data("type"));
    ...
    saveAjax: function (fileName, startTemplateUrl, callback) {
        ...
        data["fileName"] =
            fileName && fileName != ""
                ? fileName
                : Vvveb.FileManager.getCurrentUrl();
...
Vvveb.FileManager = {
        ...
        function drawComponentsTree(tree) {
            ...
            for (i in tree) {
                ...
                if (tree[i].children.length > 0) {
					...
					<label for="id' +
                            j +
                            '" style="background-image:url(libs/builder/' +
                            node.image +
                            ')"><span>' +
                            node.name +
                            '</span></label>\
    ...
    highlightComponent: function (component) {
        ...
        nodeLi.each((nl) => {
            if (nodeLi[nl]) {
                const $node = $(nodeLi[nl]);
                const dataNode = $node.data("node");
                if (dataNode) {
                    const $el = $(dataNode);
                    if ($el.is(component)) {
                        const $parents = $node.parents("li");

                        nodeLi.removeClass("active");
                        $node.addClass("active");

                        $parents.each((li) => {
                            $($parents[li])
                                .children("input[type='checkbox']:first")
                                .prop("checked", true);
                        });

                        $("#filemanager .tree").animate(
                            {
                                scrollTop: $node.offset().top,
                            },
                            1000
